...
import EscapeGameDetails from '../Escapegame/EscapegameDetails';
...
const RatingComponent = () => {
    ...
    const fetchEscapeGame = async () => {
        setloading(true);
        try {
            const escapeGame = await action.escapeGameAction.getEscapeGameById(Number(esgId));
    ...
    const fetchRating = async () => {
        try
        {
            const response = await action.ratingAction.GetAllRatingbyEscapeGameId(Number.parseInt(esgId!),page,PAGE_SIZE);
            if (response.Success) {
    ...
    const tab:TabItem[]=[
        {
            ...
            content: (
                getRating ? (
                    <>
                        <RatingTable data={getRating} onClick={handleSelect} />
                    </>
                ) : (
                    <Card>
                        <CardContent>
        ...
        {   
            label:"details",
            content: selectrating ? (<>
                <RatingDetails data={selectrating} />
            </>):(<Card>
                <CardContent>
                    <CircularProgress/>
                </CardContent>
